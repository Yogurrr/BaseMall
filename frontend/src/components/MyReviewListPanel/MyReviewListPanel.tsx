...
export const MyReviewListPanel = ({ reviews, emptyMessage }: MyReviewListPanelProps) => {
  if (reviews.length === 0) {
    return (
      ...
      {reviews.map((review) => (
        <li key={review.id} className={styles.card}>
          <Link to={`/products/${review.productId}`} className={styles.thumb}>
            <ProductThumb imageUrl={review.productImageUrl} alt={review.productName} size="lg" />
          </Link>
          <div className={styles.info}>
            <Link to={`/products/${review.productId}`} className={styles.productName}>
              {review.productName}
            </Link>

...
export const updateBadge = async (id: number, payload: BadgeInput): Promise<Badge> => {
...
export const getBadgeGradient = (badges: Badge[], name?: string | null): string => {
  const badge = badges.find((b) => b.name === name);
  return badge ? `linear-gradient(120deg, ${badge.colorFrom}, ${badge.colorTo})` : DEFAULT_BADGE_GRADIENT;
};

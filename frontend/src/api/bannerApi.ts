...
export const updateBanner = async (id: number, payload: BannerInput): Promise<Banner> => {
  const response = await api.put<Banner>(`/banners/${id}`, payload);
  return response.data;
};

export const updateBannerActive = async (id: number, active: boolean): Promise<Banner> => {
